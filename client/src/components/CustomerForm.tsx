...
const CustomerForm: React.FC<CustomerFormProps> = ({
  ...
  useEffect(() => {
    if (formData.district && districtsOptions.length > 0) {
      const selectedDistrict = districtsOptions.find(
  ...
  const handleChange = useCallback(
    (
      ...
      value: string | boolean | LocationData | null
    ) => {
      setFormData((prev) => {
    ...
    [onSaveInfoChange]
  ); // Bỏ onFormChange khỏi dependency

  // Thêm useEffect này để gọi onFormChange khi formData thay đổi
  useEffect(() => {
          ...
          >
            ...
            {!currentUser && (
              <Button
                ...
                to="/login"
                variant="text"
                sx={{ color: primaryColor, p: 0, fontWeight: "bold" }}
              >
                Đăng nhập
              </Button>
            )}
        ...
        <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
          ...
          <Grid container spacing={6}>
            <Grid item xs={12} sm={6}>
              {" "}
              {/* Sửa Grid span thành item xs={12} sm={6} */}
              <TextField
                fullWidth
                label="Họ và tên"
                variant="standard"
                ...
                sx={{ ...standardTextFieldSx, minWidth: 240 }}
                error={!!errors.name}
          ...
          <Grid container spacing={6}>
            <Grid item xs={12} sm={6}>
              <Autocomplete
                ...
                value={
                  allProvinces.find((p) => p.Name === formData.province) || null
                }
            ...
            <Grid item xs={12} sm={6}>
              <Autocomplete
                ...
                getOptionLabel={(option) => option.Name || ""}
                value={
                  districtsOptions.find((d) => d.Name === formData.district) ||
                  null
                }
          ...
          <Grid container spacing={6}>
            <Grid item xs={12} sm={6}>
              <Autocomplete
                ...
                getOptionLabel={(option) => option.Name || ""}
                value={
                  wardsOptions.find((w) => w.Name === formData.ward) || null
                }
        ...
        <FormControlLabel
          control={
            <Checkbox
              ...
              onChange={(e) =>
                handleChange("save_info_for_next_time", e.target.checked)
              }
              sx={{
                color: primaryColor,
