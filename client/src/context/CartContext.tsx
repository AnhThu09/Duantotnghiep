...
const cartReducer = (state: CartState, action: CartAction): CartState => {
  switch (action.type) {
    case "SET_CART":
      return {
        ...state,
        ...calculateDerivedState(action.payload),
        isLoading: false,
        error: null,
      };
    case "SET_LOADING":
...
export const CartProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const fetchCart = useCallback(async () => {
    ...
    try {
      ...
      dispatch({
        type: "SET_ERROR",
        payload: err.response?.data?.error || "Không thể tải giỏ hàng.",
      });
    }
  }, [currentUser]);
  ...
  const addItem = useCallback(
    async (product: ProductForCart, quantity = 1) => {
  ...
  const removeItem = useCallback(
    async (id: string) => {
      await handleApiRequest(
        () =>
          axios.delete(
            `${CART_API_BASE_URL}/${currentUser!.user_id}/${Number(id)}`
          ),
