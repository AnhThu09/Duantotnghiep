...
export default function BrandManagerr() {
  ...
  const handleEdit = useCallback((brand: Brand) => {
    setEditingBrand(brand); // Đặt đối tượng brand để chỉnh sửa
    setModalFormData(brand); // Load dữ liệu brand vào form
    setLogoPreview(brand.logo ? UPLOADS_BASE_URL + brand.logo : ''); // Hiển thị logo hiện có
    setLogoFile(null); // Reset file logo, người dùng sẽ chọn lại nếu muốn đổi
    setIsModalOpen(true);
  }, [UPLOADS_BASE_URL]);
